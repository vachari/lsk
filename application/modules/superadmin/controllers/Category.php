...
class Category extends CI_Controller {
    ...
    public function manageCategory() {
        ...
        $cols = 'menu_id,menu_title,image,menu_code,front_enable,flag_status,trash';
    ...
    public function update_menu() {
       ...
       if(empty($upload_picture)){$update_data = array('menu_title' => ucwords($this->input->post('menu_title')));}else{
      $update_data = array('menu_title' => ucwords($this->input->post('menu_title')),'image'=> $upload_picture);}
   ...
   # create submenu 
    ...
    public function managesubCategory() {
        ...
        $cols = 'menu_id,submenu_id,submenu_title,submenu_code,submenu_banner,submenu_app_icon,created_date,created_ipaddress,flag_status,priority';
        $search =  $this->input->post('search');
    ...
    public function updatesubCategory() {
       $submenuimage = $_FILES['image']['name'];
        ...
        if(empty($upload_picture)){ $upload_picture=$this->input->post('old_banner');}
       if(empty($upload_picture1)){ $upload_picture1=$this->input->post('old_app_icon');}
        $this->form_validation->set_rules('menu_id', ' Choose Menu', 'required');
        $data_submenu = array('menu_id' => $this->input->post('menu_id'),
            'submenu_title' => ucwords($this->input->post('submenu_title')),
            'submenu_banner' => $upload_picture,
            'submenu_app_icon' => $upload_picture1
        );
    ...
    public function managelistsubCategory() {
        ...
        $cols = 'listsubmenu_id,menu_id,submenu_id,listsubmenu_title,listsubmenu_banner,listsubmenu_app_icon,created_date,created_ipaddress,flag_status,priority';
        $search =  $this->input->post('search');
        $category =  $this->input->post('category');
        if($category!=''){
            $search = $this->input->post('category');
    ...
    public function updatesublistCategory(){
      ...
      if(empty($upload_picture)){ $upload_picture=$this->input->post('old_banner');}
       if(empty($upload_picture1)){ $upload_picture1=$this->input->post('old_app_ico');}
    ...
    public function updateSlider(){
      ...
      if(empty($upload_picture)){ $upload_picture=$this->input->post('old_image');}
       if(empty($upload_picture1)){ $upload_picture1=$this->input->post('old_app_icon');}
        $data_listsubmenu = array(
            'slider_title' => $this->input->post('slider_title'),
            'slider_url' => $this->input->post('slider_url'),
    ...
    public function insertProduct() {
        ...
        $imageextension = array("jpg", "JPG", "GIF", "PCO", "PNG", "png", "gif", "jPEG", "jpeg");
        if ($image != '' && $menu != '' && $submenu != '' && $listsubmenu != '' && $prodcode != '' && $prodtitle != '' &&
