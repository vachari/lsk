...
<head>
  ...
  <title><?php echo PROJECT_NAME;  ?> | Group prices</title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Manage Group Price</li>
      </ol>
    </section>
    ...
    <section class="content">
        ...
        <div class="col-md-2">
            ...
            <select class="form-control" name="group" id="group"> 
              <option value=""> Select Group</option>
                <?php $group=json_decode($group_details);
                      foreach ($group->product_details as $group) { ?>
                        <option value="<?php echo $group->id ?>"><?php  echo $group->group_name.'('.$group->group_code.')'; ?></option>
                      <?php  } ?>
      ...
      <option value="<?php echo $pro->id ?>"><?php  echo $pro->prod_name.'('.$pro->prod_code.')'; ?></option>
        ...
        <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH;?>Product/addProductGroupPricing" class="btn btn-primary pull-right">Add New</a>
        </div>
      </div>
      <div style="height: 0px;"></div><br><span id="successmessage"></span>
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        <thead>
                            ...
                            <tr>
                                ...
                                <!-- <th> Sl.No</th> -->
                                <th> Shoperative Product(code)</th>
                                ...
                                <th><span title="Discount(%)">Disc</span></th>
                        ...
                        <tbody>
                            <?php
                            $i = 1;
                            $groupprice_res = json_decode($common_result);
                            if ($groupprice_res->code == SUCCESS_CODE) {
                                foreach ($groupprice_res->common_result as $row) {
                                    ?>
                                    <tr class="">
                                ...
                                foreach ($group->product_details as $group) {
                                        if($row->prod_group==$group->id ){
                                         echo $group->group_name.'('.$group->group_code.')'; }
                                          }
                                         ?>
                                          </td>
                                        <td>
                                        <?php
                                           $units=json_decode($units_details);
                                          
                                             foreach ($units->units_details as $unit){
                                          if($row->unit_of_measure==$unit->id )
                                           {
                                             echo $unit->unit_code; 
                                            }
                                        ...
                                        <td> 
                                          ...
                                          <a href="<?php echo base_url(); ?>superadmin/Product/updateProductGroupPrice/<?php echo $row->id; ?>" class="btn btn-sm btn-info" title="Edit Product Group Price"><i class="glyphicon glyphicon-edit"> </i> </a>
                                ...
                                <tr><td colspan="11" style="text-align:center; color:red;font-size:20px;">No Results Found...</td></tr>
                            <?php }
                            ?>
      ...
      //alert(checklist);
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'group', 'updatelist': checklist, 'activity': s},
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'group','updatelist':checklist},
