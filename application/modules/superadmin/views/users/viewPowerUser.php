...
<head>
  ...
  <title><?php echo PROJECT_NAME;  ?> | Power Users </title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Manage Power User</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class="content">
      <div class="row">
        ...
        <!--  <button  class="btn btn-success   btn-md" onclick="updateActivationStatus('1')"> Approve </button>
            <button  class="btn btn-warning   btn-md"  onclick="updateActivationStatus('0')">Disapprove  </button>   -->
          <div class="pull-right">                  
           <span><button  class="btn btn-danger   btn-md" onclick="commonDelete();"> Delete </button></span>
          <a href="<?php echo base_url().'superadmin';?>" class="btn btn-primary btn-md "> Back </a>
          </div>  
        </div>
              ...
              <?php if($this->session->flashdata('success')){echo "<div class='alert alert-success  alert-dismissible text-center'>".$this->session->flashdata('success')."<a href='' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";}if($this->session->flashdata('failed')){ echo "<div class='alert alert-danger  alert-dismissible text-center'>".$this->session->flashdata('failed')."<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a></div>";} ?>
                    ...
                    <?php 
                      $followers=json_decode($followers_count);
                       $standard_res = json_decode($common_result);
                           $count = $standard_res->result_count;
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <tbody>
                            <?php
                            $i = 1;
                            $followers_limit=json_decode($followers_limit);
                         $limit_follow = $followers_limit->data_result[0]->follower_limit;
                                ...
                                foreach ($standard_res->common_result as $row) {
                                    ...
                                    <tr class="">
                                       ...
                                       <!--  <td> 
                                        ...
                                        <td><?php 
                                          $couunt=array();
                                          foreach ($followers->data_result  as $follow) {
                                              if($row->user_id == $follow->power_user_id ){
                                                // $couunt= $follow->follower_id;
                                                 $couunt []=$follow->id;
                                            }

                                          } 
                                          $folloers_count = count($couunt); ?>
                                          <a href="<?php echo base_url().'superadmin/viewfollowers/'.base64_encode($row->user_id);?>" class="btn btn-info btn-sm"> 
                                       <b><?php echo $folloers_count;?></b>&nbsp; Followers 
                                          </a>
                                       </td>
   <td><a href="<?php echo ($row->fb_link?$row->fb_link:'');?>" target="_blank"><?php echo ($row->fb_link?$row->fb_link:'');?></a></td>                                       
                                        <td> 
                                         
<?php if($limit_follow <= $followers_count )
{
if ($row->power_approved == 0 ) { ?>
	     <a href="<?php echo base_url().'superadmin/poweruser/approve/'.base64_encode($row->user_id).'/'.base64_encode($folloers_count).'/'.base64_encode('1');?>" class="btn btn-success btn-sm"> Approve </a>
	      <?php }else{ ?>
	        <a href="<?php echo base_url().'superadmin/poweruser/approve/'.base64_encode($row->user_id).'/'.base64_encode($folloers_count).'/'.base64_encode('2');?>" class="btn btn-danger btn-sm" role="button"> Disapprove </a>
	       <?php } }else{ if($row->power_approved == 0 ){echo "<div style='color:red;'>  <b>". ($limit_follow - $folloers_count) . "</b> More  Followers <br> Need to Approve </div>";}else{ echo '<button class="btn btn-default btn-sm  disabled"> Approved  </button>';} }?>
                                ...
                                <tr><td colspan="10" style="text-align:center; color:red;font-size:20px;">Results not found</td></tr>
                            <?php }
                            ?>
                        </tbody>
                    </table>
   ...
   function updateActivationStatus(s) {
      ...
      //alert(checklist);
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'power', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Users/commonStatus/',
                success: function (u) {
                    console.log(u);
                    if(u.code=='200'){
                      sendApprove(checklist);
    ...
    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');setTimeout(function() {window.location=location.href;},2000);}
     ...
     function sendApprove(id){
        jQuery.ajax({
        type: "POST",
        url: "<?php echo base_url(); ?>" + "superadmin/Users/sendApporvel",
        dataType: 'json',
        data: {'user_id':id},
        success: function(a){
          console.log(a);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'power','updatelist':checklist},
