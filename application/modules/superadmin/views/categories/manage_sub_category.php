...
<head>
  ...
  <title><?php echo PROJECT_NAME;  ?> | Manage Sub Category </title>
    ...
    <section class="content">
      ...
      <div class="col-md-12 col-sm-12 ">
        ...
        <div class="col-md-4 ">
          ...
          <div class="col-md-6 pd0 form-inline">
           ...
           <input type="search" name="search" id="search" class="form-control" placeholder="Search by Sub Cat">
           
           </div>
            <div class="col-md-2 pd0">
           <button type="submit" id="search_btn" class="btn btn-md btn-default">Search</button>
           </div>
        ...
        <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH; ?>Category/createsubCategory" class="btn btn-primary btn-md pull-right">Add New</a>
           <?php
      ...
      <div style="height: 35px;"></div>
            ...
            <div class="box-body">
               ...
               <?php if ($this->session->flashdata('Failed')) { ?>
                  ...
                  <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                    ...
                    <tbody>
                      ...
                      ?>
                      <tr class="">
                        <td><input type="checkbox" class="checkbox" name="multiple[]" value="<?php echo $row->submenu_id;?>"> </td>
                        <td><?php echo $i; ?> </td>
                        <td>  
                           <?php
                        ...
                        <td> <?php echo $row->submenu_title; ?>(<?php echo $row->submenu_code; ?>- <?php echo $row->submenu_id; ?>) </td>
                        <td>
                        <?php if(!empty($row->submenu_banner)){ ?>
                        <img src="<?php echo base_url();?>uploads/submenu/<?php echo $row->submenu_banner; ?>" style="width:75px;height:40px" /> 
                        <?php }
                         else{
                          echo "NO IMAGE"; 
                        ...
                        <td class="text-success"><b> <?php if ($row->flag_status == 1) {
                                                    echo "<b style='color:green'>Active</b>";
                                                } elseif($row->flag_status == 0) {
                                                    echo "<b style='color:red'>Inactive</b>";
                                                } ?>  </b></td>
                        <td> 
                        <a href="<?php echo base_url(); ?>superadmin/deletesubmenu/<?php echo $row->submenu_id; ?>" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure?')" title="Delete Sub Category"> <i class="glyphicon glyphicon-trash"></i> </a>
                        <a href="<?php echo base_url(); ?>superadmin/Category/get_sub_menu/<?php echo $row->submenu_id; ?>" class="btn btn-sm btn-info" title="Edit Sub Category"> <i class="glyphicon glyphicon-edit"> </i> </a>
                        </td>
                      </tr>
                      <?php 
                        $i++ ;} } else{ ?>
                          <tr><td colspan="8" style="text-align:center; color:red;font-size:20px;">Sub Categories Not Found</td></tr>
              ...
              <td colspan="8"> <p class="pages"><?php echo $links; ?></p></td>
              </div>
            </div>
            <!-- /.box-body -->
          </div>
...
<script type="text/javascript">
   function updateActivationStatus(s) {
      ...
      //alert(checklist);
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'submenu', 'updatelist': checklist, 'activity': s},
    ...
    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');setTimeout(function() {window.location=location.href;},2000);}
           ...
           $('#fail').show();$('#failmessage').html('*  Please Select records').addClass('alert alert-danger');setTimeout(function() {window.location=location.href;},2000);
        }
    }
</script>
<script type="text/javascript">
...
<script type="text/javascript">
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'submenu','updatelist':checklist},
    ...
    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');setTimeout(function() {window.location=location.href;},2000);}
       ...
       $('#fail').show();$('#failmessage').html('*  Please Select ').addClass('alert alert-danger');setTimeout(function() {window.location=location.href;},2000);
      }
     } 
    }
