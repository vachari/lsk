...
<head>
  ...
  <title><?php echo PROJECT_NAME;  ?> | Manage Units </title>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      ...
      <ol class="breadcrumb">
        ...
        <li class="active">Manage Units Of Measure</li>
      </ol>
    </section>
    ...
    <section class="content">
      ...
      <div class="col-md-12 col-sm-12 ">
        <div class="col-md-4 ">
          ...
          <div class="col-md-9  ">
       <form method="post">
           <input type="search" name="search" class="form-control" placeholder="Search by product unit">
           </div>
            <div class="col-md-2  ">
        ...
        <div class="col-md-1">
           <a href="<?php echo SUPER_ADMIN_FOLDER_PATH;?>Settings/createUnits/" class="btn btn-primary pull-right">Add New</a>
        </div>
      </div>
...
//     echo $re;
// }
$units=json_decode($common_result);
?>
                        <thead>
                        ...
                        <tbody>
                            <?php
                            $i = 1;
                            if($units->common_result !=null){
                            foreach ($units->common_result as $row) {
                                ?>
                                <tr class="">
                                    <td><input type="checkbox" class="inline-checkbox" name="multiple[]" value="<?php echo $row->id; ?>" >   </td>
                                    <td><?php echo $row->id; ?> </td>
                                    <td>  <?php echo $row->state; ?></td>
                                    <td>  <?php echo $row->quantity_of_measure; ?></td>
                                    ...
                                    <td> 
                                        <a href="<?php echo base_url(); ?>superadmin/Settings/unitDelete/<?php echo $row->id; ?>" class="btn btn-sm btn-danger " onclick="return confirm('Are you sure?')"> Delete <i class="glyphicon glyphicon-trash"></i> </a> 
                                        <a href="<?php echo base_url(); ?>superadmin/Settings/getUnitsdata/<?php echo $row->id; ?>" class="btn btn-sm btn-info "> Edit <i class="glyphicon glyphicon-edit"> </i> </a>
                                    </td>
                                </tr>
                            <?php
                            $i++;
                        } }else{ ?>
                        <tr><td colspan="8" style="text-align:center; color:red;font-size:20px;">Units Not Found</td></tr> 
                        <?php } ?>
      ...
      // alert(listarray);
        ...
        if (!isNaN(s) && (s == '1' || s == '0') && checklist != '') {
            ...
            $.ajax({
                dataType: 'json',
                type: 'post',
                data: {'tablename': 'units', 'updatelist': checklist, 'activity': s},
                url: '<?php echo base_url(); ?>superadmin/Settings/commonStatus/',
                success: function (u) {
                    console.log(u);
    ...
    var listarray=new Array();
      ...
      if(checklist!=''){
         ...
         $.ajax({
          dataType:'json',
          type:'post',
          data:{'tablename':'units','updatelist':checklist},
          url:'<?php echo base_url();?>superadmin/Settings/commonDelete/',
          success:function(u){
           // console.log(u);
            if(u.code=='200'){$('#success').show();$('#successmessage').html(u.description).addClass('alert alert-success');setTimeout(function() {window.location=location.href;},2000);}
                    if(u.code=='204' || u.code=='301' || u.code=='422'){$('#fail').show();$('#failmessage').html(u.description).addClass('alert alert-danger');}
