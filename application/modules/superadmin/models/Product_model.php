...
{
  $this->db->select("id,vendor_id,vendor_item_code,prod_code,prod_group,sku,unit,prod_name,prod_desc,hsn_code,shelf_life_no,shelf_life_unit,category,sub_category,listsubmenu_id,prod_image,other_image")->from("ga_main_prod_details_tbl")->where("id",$id);
  $query=$this->db->get();
  $count=$query->num_rows();
  return ($count > 0)?$query->row():array();
}
